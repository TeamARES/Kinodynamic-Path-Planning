...
#for python3 replace input() with input()
max_velx = 3
min_velx = -3
max_vely = 3
min_vely = -3
...
def path_planning(x_in, y_in, vx, vy, gx, gy, vgx, vgy, ox, oy):
    ...
    while 1:
        ...
        #access finalvx and finalvy from here
      
        if rx==gx and ry==gy and ggkk.vx == vgx and ggkk.vy == vgy:
            break
        #function to change linear and angular velocity based on finalvx, finalvy whenever its made, here
        linear_velocity=2;
        angular_velocity=3;
        ...
        '''
        #need a socket here for obstacle revision
        ox,oy = revise_obstacle_coordinates(ox,oy)
        print("Enter 1 for location feedback")
        #need a socket here to enter location feedback status
        feed_loc = int(input())
        if feed_loc is 1:
